...
class AnalyseController extends Controller
{
    ...
    public function donnees(Request $request)
    {
        ...
        foreach ($collectes as $collecte) {
            $indicateursData = json_decode($collecte->donnees, true);
    ...
    private function determinerCategorieEtNom($key)
    {
        ...
        $indicateur = Collecte::where('donnees', $key)->first();
    ...
    /**
     * Calculer la tendance d'un indicateur en comparant avec les données précédentes
     *
     * @param string $indicateur Code de l'indicateur
     * @param float $valeurActuelle Valeur actuelle de l'indicateur
     * @param int $exerciceId ID de l'exercice courant
     * @param int $periodeId ID de la période courante
...
private function calculerTendance(string $indicateur, $valeurActuelle, int $exerciceId, ?int $periodeId, int $entrepriseId): string
{
    ...
    if (!$collectePrecedente || !$collectePrecedente->donnees) {
        return 'stable';
    }

    $donneesPrecedentes = is_string($collectePrecedente->donnees)
        ? json_decode($collectePrecedente->donnees, true)
        : $collectePrecedente->donnees;
