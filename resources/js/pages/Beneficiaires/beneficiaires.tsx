...
const Promoteurs = () => {
    ...
    //     }
    // );
// Préparer une version en minuscule du terme recherché (optimisation)
...
const filteredBeneficiaires = beneficiaires.filter((beneficiaire) => {
    // Filtrage par type de promoteur
    const matchesType =
        activeTypeFilter === 'tous' || beneficiaire.type_beneficiaire === activeTypeFilter;

    // Filtrage par recherche (avec fallback sécurisé)
    const matchesSearch =
        (beneficiaire.nom?.toLowerCase() || '').includes(search) ||
        (beneficiaire.prenom?.toLowerCase() || '').includes(search) ||
        (beneficiaire.contact || '').includes(searchTerm) || // contact = numéro → pas besoin de minuscule
        (beneficiaire.email?.toLowerCase() || '').includes(search) ||
        (beneficiaire.regions?.toLowerCase() || '').includes(search) ||
        (beneficiaire.village?.toLowerCase() || '').includes(search);
