...
// //         const draftIds = collectes.data.filter((c) => c.type_collecte === 'brouillon').map((c) => c.id);
...
// //                                             )}
// //                                             {visibleColumns.user && (
// //                                                 <th className="w-[20%] px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase dark:text-gray-300">
// //                                                     Utilisateur
// //                                                 </th>
// //                                             )}
...
// //                                                         {collecte.periode
// //                                                             ? typeof collecte.periode === 'string'
// //                                                                 ? collecte.periode === 'Exceptionelle'
// //                                                                     ? 'Exceptionelle'
// //                                                                     : collecte.periode
// //                                                                 : collecte.periode.nom || collecte.periode.type_periode
// //                                                             : 'Exceptionelle'}
...
// import React, { useEffect, useState } from 'react';
// import { toast } from 'sonner';
// // Importer le nouveau composant pour le bouton "Nouvelle Collecte" hors ligne
// import NouvelleCollecteButton from '@/components/NouvelleCollecteButton';
...
//     // Gérer la taille de l'écran pour le responsive
//     useEffect(() => {
//         const handleResize = () => {
...
//                     },
//                     periode: periodeObj || `Période #${local.periode_id}`,
//                     date_collecte: local.date_collecte || new Date().toISOString(),
//                     type_collecte: local.is_draft ? 'brouillon' : 'standard',
//                     donnees: local.donnees || {},
//                     created_at: new Date(local.timestamp || Date.now()).toISOString(),
//                     user: {
//                         id: 0,
//                         name: 'Utilisateur local',
//                     },
...
//     // Obtenir la fonction de suppression depuis le hook au niveau du composant
//     const { deleteLocalCollecte: removeLocalCollecte } = useOfflineStorage();
...
//         // Filtrer les collectes à imprimer
//         const collectesToPrint = selectedCollectes.length > 0 ? mergedCollectes.filter((c) => selectedCollectes.includes(c.id)) : mergedCollectes;
...
//                 : ''
//         }
//         ${visibleColumns.date ? `<td>${formatDateTime(collecte.created_at)}</td>` : ''}
//         ${visibleColumns.status ? `<td class="${collecte.type_collecte === 'brouillon' ? 'draft' : 'standard'}">${collecte.type_collecte === 'brouillon' ? 'Brouillon' : 'Standard'}</td>` : ''}
//         ${visibleColumns.user ? `<td>${collecte.user ? collecte.user.name : 'N/A'}</td>` : ''}
//         ${visibleColumns.created_at ? `<td>${formatDateTime(collecte.created_at)}</td>` : ''}
//         <td>${collecte.is_local ? 'Local' : 'Serveur'}</td>
...
//     // Nombre total de collectes en incluant celles en mémoire locale
//     const totalCollectes = (collectes?.total || 0) + localCollectes.filter((lc) => !lc.synced).length;

//     // Nombre de collectes locales en attente
//     const pendingLocalCollectes = localCollectes.filter((c) => !c.synced).length;

//     // Rendu de la table pour appareils mobiles
...
//                             selectedCollectes.includes(collecte.id) ? 'border-l-4 border-blue-500' : ''
//                         } ${collecte.is_local ? 'border-r-4 border-indigo-500' : ''}`}
//                     >
//                         <div className="flex items-start justify-between">
//                             <div className="flex items-center">
//                                 <input
//                                     type="checkbox"
//                                     className="mr-3 rounded-md border-2 border-gray-300 text-blue-600 shadow-sm"
//                                     checked={selectedCollectes.includes(collecte.id)}
//                                     onChange={() => toggleSelect(collecte.id)}
//                                 />
//                                 <div>
//                                     <h3 className="font-semibold text-gray-800 dark:text-white">{collecte.entreprise.nom_entreprise}</h3>
...
//                                 <span className="text-gray-800 dark:text-gray-200">{formatDateTime(collecte.created_at)}</span>
//                             </div>
...
//                                 <span className="text-gray-800 dark:text-gray-200">{collecte.user ? collecte.user.name : 'N/A'}</span>
//                             </div>
//                             <div>
//                                 {collecte.is_local ? (
//                                     <span className="inline-flex items-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-300">
...
//     // Rendu de la table pour desktop
//     const renderDesktopTable = () => {
//         return (
//             <div className="overflow-x-auto rounded-lg border-2 border-gray-200 shadow-lg dark:border-gray-700">
//                 <table className="w-full min-w-[1000px] divide-y divide-gray-300 dark:divide-gray-600">
//                     <thead className="bg-gray-50 dark:bg-gray-700">
//                         <tr>
//                             <th className="w-10 px-4 py-3">
//                                 <input
...
//             <Head title="Liste des collectes" />
//             <Toaster position="top-right" richColors />
//             <div className={`py-6 transition-colors duration-200 md:py-12 ${darkMode ? 'dark' : ''}`}>
//                 <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
//                     <div className="overflow-hidden bg-white p-4 shadow-lg transition-colors sm:rounded-xl md:p-6 dark:bg-gray-800">
//                         {/* En-tête et actions principales */}
//                         <div className="relative mb-6 flex flex-col gap-4">
//                             <div className="mb-2 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
//                                 <div>
//                                     <h1 className="text-xl font-semibold text-gray-800 md:text-2xl dark:text-white">Liste des Collectes</h1>
//                                     <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
//                                         Total: {totalCollectes} collecte(s), dont {totalDrafts} brouillon(s)
//                                         {pendingLocalCollectes > 0 && ` et ${pendingLocalCollectes} en attente de synchronisation`}
//                                     </p>
...
//                                 {/* Remplacement du Link par le nouveau composant NouvelleCollecteButton */}
//                                 <NouvelleCollecteButton />

//                                 <FormulaireExceptionnelButton beneficiaires={beneficiaires} exercices={exercices} variant="secondary" />
//                             </div>

//                             {/* Deuxième ligne : Autres boutons */}
//                             <div className="mb-4 flex flex-col flex-wrap items-center justify-end gap-4 sm:flex-row">
//                                 <button
//                                     onClick={() => setShowFilters(!showFilters)}
//                                     className="inline-flex w-full items-center justify-center rounded-lg border-2 border-gray-300 bg-gray-200 px-5 py-3 text-gray-700 shadow-md transition hover:bg-gray-300 sm:w-auto dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"
//                                 >
//                                     <FilterIcon className="mr-2 h-4 w-4" />
//                                     {showFilters ? 'Masquer les filtres' : 'Filtres'}
//                                 </button>
...
//                                     type="button"
//                                     onClick={() => exportData('excel')}
//                                     className="flex w-full items-center justify-center rounded-md bg-green-600 px-5 py-3 text-white shadow-md transition hover:bg-green-700 sm:w-auto"
//                                 >
...
//                                 <button
//                                     onClick={() => exportData('pdf')}
//                                     className="flex w-full items-center justify-center rounded-md bg-red-600 px-5 py-3 text-white shadow-md transition hover:bg-red-700 sm:w-auto"
//                                 >
...
//                                 <button
//                                     onClick={printCollectes}
//                                     className="flex w-full items-center justify-center rounded-md bg-blue-600 px-5 py-3 text-white shadow-md transition hover:bg-blue-700 sm:w-auto"
//                                 >
...
//                         {/* Filtres */}
//                         {showFilters && (
//                             <div className="mb-6 rounded-lg border-2 border-gray-200 bg-gray-50 p-4 shadow-inner md:p-6 dark:border-gray-600 dark:bg-gray-700">
//                                 <div className="mb-4 flex items-center justify-between">
//                                     <h2 className="text-lg font-medium text-gray-800 dark:text-white">Filtres</h2>
//                                     <button
//                                         onClick={resetFilters}
...
//                         {/* Tableau des collectes - adapté aux mobiles et desktop */}
//                         {mergedCollectes.length > 0 ? (
//                             <>
//                                 {/* Version mobile - affichée sur petits écrans */}
//                                 <div className="md:hidden">{renderMobileTable()}</div>

//                                 {/* Version desktop - cachée sur petits écrans */}
...
//                         {/* Pagination - rendue responsive */}
//                         {collectes?.links && collectes.links.length > 3 && (
//                             <div className="mt-6">
//                                 <nav className="flex flex-col items-center justify-between gap-4 sm:flex-row">
//                                     <div className="w-full text-center text-sm text-gray-700 sm:w-auto sm:text-left dark:text-gray-300">
//                                         Affichage <span className="font-medium">{collectes.from || 0}</span> à{' '}
//                                         <span className="font-medium">{collectes.to || 0}</span> sur{' '}
//                                         <span className="font-medium">{collectes.total || 0}</span> résultats
//                                         {pendingLocalCollectes > 0 && (
//                                             <span className="ml-2 block text-indigo-600 sm:inline dark:text-indigo-400">
//                                                 + {pendingLocalCollectes} collecte(s) locale(s)
//                                             </span>
//                                         )}
//                                     </div>
