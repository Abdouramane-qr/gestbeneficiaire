...
interface IndicateurRHData {
  [key: string]: string;
}

interface RHTabProps {
  data: IndicateurRHData;
  onChange: (data: IndicateurRHData) => void;
  errors: string[];
}

const RHTab: React.FC<RHTabProps> = ({ data, onChange, errors }) => {
  // Obtenir la définition des champs pour les indicateurs RH
  const fields = IndicateurCalculator.getFieldsByCategory('rh');

  // Suivi local des valeurs des champs
  const [values, setValues] = useState<IndicateurRHData>(data);
