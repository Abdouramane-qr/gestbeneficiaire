...
interface RolesPermissionsManagementProps {
  roles: Role[];
  modules: Module[];
  actions: Action[];
}
...
const RolesPermissionsManagement = ({ roles, modules, actions }: RolesPermissionsManagementProps) => {
  ...
  // Helper function to determine if all actions for a module are selected
  const areAllActionsSelected = (moduleId: string) => {
    return actions.every(action =>
      data.permissions[moduleId]?.includes(action.id)
    );
  };
  ...
  // Helper function to toggle all actions for a module
  const toggleAllActionsForModule = (moduleId: string) => {
    if (areAllActionsSelected(moduleId)) {
      // If all selected, deselect all
      setData(prevData => {
        const updatedPermissions = { ...prevData.permissions };
  ...
  if (!selectedRole) {
    return (
      <div className="flex flex-col h-full">
        <div className="bg-blue-600 text-white p-4">
          <h1 className="text-2xl font-bold">Gestion des Rôles et Accès</h1>
          <p className="text-sm text-blue-100">Aucun rôle trouvé. Veuillez créer un rôle.</p>
        </div>
        <div className="p-4">
          <button
            onClick={handleAddNewRole}
        ...
        <Modal isOpen={isModalOpen} onClose={handleCloseModal} children={undefined}>
          {/* Modal content */}
        </Modal>
      </div>
    );
